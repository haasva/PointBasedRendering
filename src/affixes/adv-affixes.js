export async function generateAdventurerAffixes(adventurer) {
  try {
...
adventurer.BaseRarity = adventurer.Rarity;
adventurer.Rarity = getRandomAdventurerRarity(adventurer.BaseRarity);

// Set the number of affixes based on Rarity
const randomAffixNumber = getRandomAffixCount(adventurer.Rarity);
    ...
    for (let i = 0; i < randomAffixNumber; i++) {
      ...
      const matchingAffixes = affixes.filter(affix => {
        const adventurerTypeWords = adventurer.Type.split(',').map(word => word.trim());
    ...
    setAdventurerSkillSlots(adventurer);
    applyAffixesToIndividualAdventurer(adventurer);
